    ...
    <div style={{ display:"flex", gap:10, alignItems:"flex-start", flexWrap:"wrap" }}>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
        {times.map(function(t, i) {
          return (
            <span key={i} style={{ padding:"3px 11px", borderRadius:20, background:"var(--inset)", border:"1px solid var(--stroke-1)", fontSize:12, color:"var(--text-2)" }}>{t}</span>
          );
        })}
      </div>
    </div>
    ...
    <div className="ci-work" style={{ "--ci-accent": m.accentFrom, "--ci-glow": m.accentGlow }}>
      ...
      <PLB mood={mood} title="Ask a question" desc="Click a quick question below or type your own">
        ...
        {askState === "done" && answer && (
          <div style={{ marginTop:14, padding:"15px 16px", borderRadius:12, background:"var(--inset)", border:"1px solid " + m.accentGlow + "30", fontSize:13.5, color:"var(--text-1)", lineHeight:1.7, whiteSpace:"pre-wrap" }}>
        ...
        <div style={{ padding:"0 16px 16px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {analyticsRows.map(function(row, i) {
              return (
                <div key={i} style={{ display:"grid", gridTemplateColumns:"3fr 3fr 3fr", gap:8, padding:"10px 12px", borderRadius:10, background:"var(--inset)", border:"1px solid var(--stroke-1)" }}>
