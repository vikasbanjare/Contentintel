...
async function callClaudeOnce({ system, userText, image, images, model, maxTokens = 1800 }) {
  ...
  const key = getKey();
  // Neither the BYO-key nor the sandbox path attaches a web_search tool (only the
  // hosted worker does). Several prompts still say "you have a web_search tool", so
  // the model emits <function_calls> XML as TEXT and it leaks to the UI. Tell it the
  // truth here so it writes from its own knowledge and never fakes a tool call.
  ...
  if (typeof system === "string") system = system + NO_TOOL_NOTE;
...
function KeyModal({ open, onClose }) {
  ...
  if (!open) return null;
  function save() { setKeyLS(key.trim()); setGoogleKeyLS(gkey.trim()); setOpenAIKeyLS(okey.trim()); setNvidiaKeyLS(nvkey.trim()); setReveKeyLS(rvkey.trim()); setProxyUrlLS(proxy.trim()); setModelLS(model); onClose(true); }
