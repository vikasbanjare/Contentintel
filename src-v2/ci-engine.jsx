...
window.CI_BUILD = "2026-06-20-r24";
...
// Analysis provider: "claude" (default, paid) or "groq" (free, Llama 3.3 70B via OpenAI-compatible API).
...
function canRunAnalysis() {
  if (getProvider() === 'groq') return !!getGroqKey();
  return canRun();
}
...
function useAnalysis(type, opts = {}) {
  ...
  async function run({ userText, image, images, maxTokens, system }) {
    ...
    if (!canRunAnalysis()) {
      if (hasSandbox() && getProvider() !== 'groq') {
    ...
    try {
      const { text, usage, sources: rawSources } = getProvider() === 'groq'
      ...
      if (String(e.message) === "NO_KEY" || String(e.message) === "NO_GROQ_KEY") { setTimeout(() => { if (mountedRef.current) setState("done"); }, 600); return; }
...
function AnalyzeButton({ mood, onClick, loading, estIn, estOut, label = "Analyze", model, disabled, disabledHint }) {
  ...
  const isGroq = getProvider() === 'groq';
  const cost = isGroq ? 0 : estCost(inTok, outTok, model);
  const hasKey = !!getKey();
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
      ...
      <span className="ci-est">
        {disabled && disabledHint
          ...
          : isGroq
            ? <span style={{ color: "#8FD86A" }}>Free — Groq Llama 3.3 70B</span>
            : <>est. {fmtTokens(inTok)} in + ~{fmtTokens(outTok)} out · ~{fmtCost(cost)}</>}
        {free && <span style={{ color: "var(--text-4)" }}> · live</span>}
        {!isGroq && !hasKey && !free && !(typeof window !== 'undefined' && window.CI_SESSION && (window.CI_SAAS||{}).workerUrl) && <span style={{ color: "var(--text-4)" }}> · preview — connect a key for live results</span>}
      </span>
    </div>
  );
}
...
function UsageBadge({ usage, model }) {
  ...
  const isGroq = getProvider() === 'groq';
  const costStr = isGroq ? "free (Groq)" : "~" + fmtCost(estCost(inT, outT, model));
  return (
...
function KeyModal({ open, onClose }) {
  ...
  React.useEffect(() => {
    if (open) { setKey(getKey()); setGkey(getGoogleKey()); setOkey(getOpenAIKey()); setNvkey(getNvidiaKey()); setRvkey(getReveKey()); setProxy(getProxyUrl()); setModel(getModel()); setWebSearch(getWebSearch()); const yk = getYouTubeKey ? getYouTubeKey() : ''; setYtKey(yk === '__proxy__' ? '' : yk); setGroqKey(getGroqKey ? getGroqKey() : ''); setProvider(getProvider()); }
  }, [open]);
  if (!open) return null;
  function save() { setKeyLS(key.trim()); setGoogleKeyLS(gkey.trim()); setOpenAIKeyLS(okey.trim()); setNvidiaKeyLS(nvkey.trim()); setReveKeyLS(rvkey.trim()); setProxyUrlLS(proxy.trim()); setModelLS(model); setWebSearchLS(webSearch); if (!saasMode && ytKey.trim()) setYouTubeKeyLS(ytKey.trim()); setGroqKeyLS(groqKey.trim()); setProviderLS(provider); onClose(true); }
      ...
      <div className="ci-modal" onClick={e => e.stopPropagation()}>
        ...
        {section === "analysis" && <>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[
              { id: "claude", label: "Claude (Anthropic)", desc: "Paid · highest quality" },
              { id: "groq",   label: "Groq — Llama 3.3 70B", desc: "Free · 30 req/min" },
            ].map(p => (
