...
async function callClaude({ system, userText, image, images, model, maxTokens = 1800 }) {
  ...
  const content = imgs.length
    ...
    : userText;
  // Wrap system string in a cacheable block so repeated calls for the same content
  // type reuse the cached prefix (saves ~90% on input tokens after the first call).
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: model || getModel(), max_tokens: maxTokens, system: systemBlocks, messages: [{ role: "user", content }] }),
  });
  if (!res.ok) {
    let detail = "";
