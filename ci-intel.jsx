...
// Distil a fresh sweep into compact per-area guidance and store it so every
// tool's prompt (thumbnail / title / platform) reflects this week's findings.
// Same contract as the Worker's server-side distillation — one cheap call.
const INTEL_DISTILL_SYS = [
  ...
  'Only keep findings that are actionable and likely durable (weeks, not hours); drop one-off news and speculation. Merge duplicates.',
...
async function intelDistillClient(out) {
  const lines = [];
  for (const d of INTEL_DOMAINS) {
    const r = out[d.id];
    for (const it of ((r && r.items) || [])) lines.push(`[${d.label}] ${it.headline || ''} :: ${it.what || ''} :: ${it.action || ''}`);
  }
  if (!lines.length) return;
  const corpus = 'DIGEST ITEMS:\n' + lines.join('\n').slice(0, 11000) + '\n\nDistil now.';
  let text = '';
  const gKey = window.getGoogleKey && window.getGoogleKey();
  if (gKey) {
    const res = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent', {
      ...
      body: JSON.stringify({
        systemInstruction: { parts: [{ text: INTEL_DISTILL_SYS }] },
    ...
    const r = await window.callClaude({ system: INTEL_DISTILL_SYS, userText: corpus, maxTokens: 900, temperature: 0.2 });
    text = r.text || '';
  }
  const j = (window.parseReport || (() => null))(text);
  if (j && typeof j === 'object' && window.setLiveIntel) window.setLiveIntel(j);
}
...
// System health — surfaces problems BEFORE users hit them. Passive checks are
// free (key presence, digest freshness); active tests make one real tiny call
// each so "it says OK" means "it actually worked just now".
function IntelHealthPanel({ m, serverFed, digestTs, liveIntel, onOpenKey }) {
  const [tests, setTests] = React.useState({}); // id -> 'run' | 'ok' | error string
  const gKey = window.getGoogleKey && window.getGoogleKey();
  const imgReady = !!(gKey || (window.getOpenAIKey && window.getOpenAIKey()) || (window.getProxyUrl && window.getProxyUrl()));
  const textReady = !!(window.canRun && window.canRun());
  const digestFresh = digestTs && (Date.now() - digestTs) < 36 * 3600e3;

  const setT = (id, v) => setTests(p => ({ ...p, [id]: v }));

  async function testText() {
    setT('text', 'run');
    try {
      const r = await window.callClaude({ system: 'Reply with exactly: OK', userText: 'ping', maxTokens: 8, temperature: 0 });
      setT('text', /ok/i.test(r.text || '') ? 'ok' : 'Unexpected reply — engine may be degraded.');
    } catch (e) { setT('text', String(e.message || 'failed')); }
  }
  async function testGemini() {
    setT('gemini', 'run');
    try {
      const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models?pageSize=1', { headers: { 'x-goog-api-key': gKey } });
      setT('gemini', r.ok ? 'ok' : ('Key check failed (HTTP ' + r.status + ') — check the key in Settings.'));
    } catch (e) { setT('gemini', String(e.message || 'failed')); }
  }
  async function testImage() {
    setT('image', 'run');
    try {
      const url = await window.generateImageInApp('A single bold red circle centered on a plain white background. Sharp focus, high contrast, one clear focal point.', '1:1');
      setT('image', url ? 'ok' : 'No image returned.');
    } catch (e) { setT('image', String(e.message || 'failed')); }
  }

  const Row = ({ label, status, note, action, actionLabel }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid var(--stroke-1)' }}>
      <span style={{ fontSize: 14, width: 20, textAlign: 'center', flexShrink: 0 }}>
        {status === 'ok' ? '✅' : status === 'warn' ? '⚠️' : status === 'bad' ? '❌' : status === 'run' ? '⏳' : '▫️'}
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text-1)' }}>{label}</div>
        {note && <div style={{ fontSize: 11, color: status === 'bad' ? '#F06A7E' : 'var(--text-4)', marginTop: 1, lineHeight: 1.45 }}>{note}</div>}
      </div>
      {action && <button className="ci-copybtn" style={{ height: 26, padding: '0 10px', fontSize: 11, flexShrink: 0 }} onClick={action} disabled={status === 'run'}>{actionLabel}</button>}
    </div>
  );

  const st = (id, readyFallback) => tests[id] === 'run' ? 'run' : tests[id] === 'ok' ? 'ok' : tests[id] ? 'bad' : readyFallback;

  return (
    <div className="ci-block" style={{ padding: 20, marginTop: 14 }}>
      <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-1)', marginBottom: 2 }}>🩺 System health</div>
      <div style={{ fontSize: 11.5, color: 'var(--text-4)', marginBottom: 8 }}>Catch problems before they cost you a generation. Tests make one real (tiny) call each.</div>
      <Row label="Daily auto-sweep (server cron)"
        status={serverFed || digestFresh ? 'ok' : digestTs ? 'warn' : 'warn'}
        note={digestTs ? ('Latest digest ' + intelAgo(digestTs) + (digestFresh ? ' — healthy.' : ' — older than 36h; check the Worker cron trigger.')) : 'No digest yet — the first cron tick (or a manual sweep) will create one.'} />
      <Row label="Live intel → tools injection"
        status={liveIntel ? 'ok' : 'warn'}
        note={liveIntel ? ('Feeding all prompts · updated ' + intelAgo(liveIntel.ts)) : 'Not active yet — completes automatically after the next sweep (server or manual).'} />
      <Row label="AI engine (analysis & scoring)"
        status={st('text', textReady ? undefined : 'bad')}
        note={tests.text && tests.text !== 'ok' && tests.text !== 'run' ? tests.text : textReady ? 'Configured.' : 'No engine — sign in or add a key in Settings.'}
        action={textReady ? testText : onOpenKey} actionLabel={textReady ? (tests.text === 'run' ? '…' : 'Test') : 'Fix'} />
      <Row label="Gemini key (search & images)"
        status={st('gemini', gKey ? undefined : 'warn')}
        note={tests.gemini && tests.gemini !== 'ok' && tests.gemini !== 'run' ? tests.gemini : gKey ? 'Key present.' : 'No Google key — sweeps fall back to Claude; image gen needs it.'}
        action={gKey ? testGemini : onOpenKey} actionLabel={gKey ? (tests.gemini === 'run' ? '…' : 'Test') : 'Fix'} />
      <Row label="Image generation"
        status={st('image', imgReady ? undefined : 'bad')}
        note={tests.image && tests.image !== 'ok' && tests.image !== 'run' ? tests.image : imgReady ? 'Configured. Test generates 1 tiny image (uses quota).' : 'No image key — add Google or OpenAI in Settings.'}
        action={imgReady ? testImage : onOpenKey} actionLabel={imgReady ? (tests.image === 'run' ? '…' : 'Test (1 image)') : 'Fix'} />
    </div>
  );
}
...
function IntelTab({ onOpenKey }) {
  ...
  const [feedTick, setFeedTick] = React.useState(0); // re-render when live intel updates
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Server-distilled knowledge -> feeds every tool's prompt automatically.
        if (data.knowledge && window.setLiveIntel) {
          const kTs = new Date(data.knowledge_updated_at || data.created_at || 0).getTime() || Date.now();
          const cur = window.getLiveIntel && window.getLiveIntel();
          if (!cur || kTs > cur.ts) { window.setLiveIntel(data.knowledge, kTs); setFeedTick(t => t + 1); }
        }
      } catch (e) {}
    })();
    return () => { live = false; };
  }, []);
  const liveIntel = (window.getLiveIntel && window.getLiveIntel()) || null;
  ...
  async function sweep() {
    ...
    if (!liveRef.current) return;
    // Close the loop: distil the fresh sweep into per-area guidance that every
    // tool's prompt picks up automatically. Non-fatal if it fails.
    setPhase('Distilling insights');
    try { await intelDistillClient(out); setFeedTick(t => t + 1); } catch (e) {}
    if (!liveRef.current) return;
    setPhase('');
    setRunning(false);
    const now = Date.now();
  ...
  return (
    <div className="ci-work" style={{ '--ci-accent': m.accentFrom, '--ci-glow': m.accentGlow }}>
      ...
      <div className="ci-block" style={{ padding: 20, marginTop: 22 }}>
        ...
        {liveIntel && (
          <div data-feedtick={feedTick} style={{ marginTop: 12, padding: '10px 14px', borderRadius: 10, background: 'rgba(143,216,106,0.08)', border: '1px solid rgba(143,216,106,0.3)', fontSize: 12.5, color: 'var(--text-2)', lineHeight: 1.55 }}>
            <b style={{ color: '#8FD86A' }}>🧠 Live intel is feeding your tools</b> — the thumbnail checker, title scorer, generators and platform advice now include these findings automatically (updated {intelAgo(liveIntel.ts)}).
          </div>
        )}
      ...
      <IntelHealthPanel m={m} serverFed={serverFed} digestTs={ts} liveIntel={liveIntel} onOpenKey={onOpenKey} />
