...
export default {
  async fetch(req, env) {
    ...
    // Research payload: the evaluation methodology every tool runs on. The
    // client has always requested this ({research:true}); the handler was
    // missing, so every tool silently fell back to one-line built-in stubs.
    if (body.research === true) return json({ research: CI_RESEARCH_DATA }, 200, cors);
    ...
    // 5. Call the AI. COST CONTROL: free-tier ('quick') text traffic routes to a
    // FREE provider when one is configured — GROQ_KEY first, else GEMINI_KEY —
    // so free users cost $0 in Anthropic spend. Claude serves the paid engines
    // and is the automatic fallback if the free provider errors.
    const { engine: _e, ...rest } = body;
    const maxTok = Math.min(parseInt(rest.max_tokens, 10) || 2000, MAX_OUTPUT_TOKENS);
    if (tier === 'quick' && !hasImages && (env.GROQ_KEY || env.GEMINI_KEY)) {
      try {
        const freeOut = env.GROQ_KEY ? await freeViaGroq(env, rest, maxTok) : await freeViaGemini(env, rest, maxTok);
        if (freeOut) return json(freeOut, 200, cors);
    ...
    // 6. RESILIENCE: a billing/credit failure on the host's Anthropic account
    // must not take the whole product down. If Anthropic rejects for billing /
    // quota / rate-limit reasons and a free provider is configured, serve the
    // request from that instead. Vision can't be rescued (free text models
    // can't see images) — those return the honest error and refund.
    if (!aRes.ok && !hasImages && (env.GROQ_KEY || env.GEMINI_KEY)) {
      const isBilling = aRes.status === 400 || aRes.status === 402 || aRes.status === 429 ||
                        /credit balance|billing|quota|insufficient/i.test(out || '');
      if (isBilling) {
        try {
          const rescued = env.GROQ_KEY ? await freeViaGroq(env, rest, maxTok) : await freeViaGemini(env, rest, maxTok);
          if (rescued) return json(rescued, 200, cors);
        } catch (e) { /* fall through to the original error */ }
      }
    }

    // 7. Refund the reservation if the call did not succeed.
...
// ── Free-provider bridges (zero Anthropic spend) ─────────────────────────────
// Translate the app's Anthropic-shaped request to the free provider and the
// reply back into the Anthropic response shape the client already parses.
function flattenText(c) {
  if (!c) return '';
  if (typeof c === 'string') return c;
  if (Array.isArray(c)) return c.map(b => (b && typeof b.text === 'string') ? b.text : '').filter(Boolean).join('\n');
  return typeof c.text === 'string' ? c.text : '';
}

async function freeViaGroq(env, rest, maxTok) {
  const sys = flattenText(rest.system);
  const userText = (rest.messages || []).map(m => flattenText(m.content)).filter(Boolean).join('\n\n');
  ...
  return {
    ...
    model: 'free/' + (env.GROQ_MODEL || 'llama-3.3-70b-versatile'),
    stop_reason: 'end_turn',
  };
}

async function freeViaGemini(env, rest, maxTok) {
  const sys = flattenText(rest.system);
  const userText = (rest.messages || []).map(m => flattenText(m.content)).filter(Boolean).join('\n\n');
  if (!userText) throw new Error('empty prompt');
  const mdl = env.GEMINI_TEXT_MODEL || 'gemini-2.5-flash-lite';
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${mdl}:generateContent`, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { maxOutputTokens: maxTok, ...(rest.temperature != null ? { temperature: rest.temperature } : {}) },
    }),
  });
  if (!res.ok) throw new Error('gemini ' + res.status);
  const d = await res.json();
