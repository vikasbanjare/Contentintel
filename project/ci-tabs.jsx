...
function ThumbnailTab({ onOpenKey }) {
  ...
  const [genModelId, setGenModelId] = React.useState(IMG_MODELS[0].id);
  const [genPrompt, setGenPrompt] = React.useState('');
  const [gen, setGen] = React.useState({ loading: false, out: null, err: '', via: '', surface: '' });
  React.useEffect(() => { setGen({ loading: false, out: null, err: '', via: '', surface: '' }); }, [report]);
  const genModel = IMG_MODELS.find(x => x.id === genModelId) || IMG_MODELS[0];
  async function runGenerate(basePrompt, surface) {
    if (genModel.provider === 'gemini' && !imgA) { setGen({ loading: false, out: null, err: 'Upload an image first (Gemini edits your image).', via: '', surface }); return; }
    ...
    try {
      ...
      setGen({ loading: false, out, err: '', via, surface });
    } catch (e) {
      const em = String(e.message);
      const msg = (em === 'NO_GOOGLE_KEY' || em === 'NO_NV_KEY')
        ? 'Add an NVIDIA (FLUX) or Google AI key in Settings (top-right) for this model.'
        : (e.message || 'Could not generate — try again.');
      setGen({ loading: false, out: null, err: msg, via: '', surface });
    }
  }
  const ModelPicker = () => (
    <select className="ci-input" value={genModelId} onChange={e => setGenModelId(e.target.value)} style={{ appearance: 'auto', maxWidth: 380 }}>
      {IMG_MODELS.map(mm => <option key={mm.id} value={mm.id}>{mm.label}</option>)}
    </select>
  );
  const GenResult = ({ surface }) => (gen.surface === surface ? (
    <div>
  ...
  return (
    <div className="ci-work" style={{ '--ci-accent': m.accentFrom, '--ci-glow': m.accentGlow }}>
      ...
      <TB mood={mood}>
        ...
        {!compare && imgA && (
          <details className="ci-collapse" style={{ marginTop: 14, border: `1px solid ${m.accentGlow}`, borderRadius: 12, background: `linear-gradient(135deg, ${m.orbB}33, var(--surface-1))` }}>
            <summary style={{ cursor: 'pointer', padding: '13px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontWeight: 700, fontSize: 13.5, color: 'var(--text-1)' }}>
              <span>✨ Just regenerate from a prompt — skip the check</span>
              <span className="ci-collapse-caret" style={{ opacity: 0.5, fontSize: 11.5 }}>open ▾</span>
            </summary>
            <div style={{ padding: '0 16px 16px' }}>
              <textarea className="ci-textarea" style={{ minHeight: 64 }} value={genPrompt} onChange={e => setGenPrompt(e.target.value)}
                placeholder="Describe the new version… e.g. 'add a shocked face on the right, huge yellow ₹500 Cr, dark navy background'" />
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginTop: 10 }}>
                <ModelPicker />
                <window.GlowButton mood={mood} onClick={() => runGenerate(genPrompt.trim() || 'Improve this thumbnail for higher click-through.', 'inline')}>
                  {gen.loading && gen.surface === 'inline' ? <><span style={{ display: 'inline-block', width: 13, height: 13, border: '2px solid currentColor', borderRightColor: 'transparent', borderRadius: '50%' }} className="spin" /> Generating…</> : 'Generate →'}
                </window.GlowButton>
              </div>
              <GenResult surface="inline" />
            </div>
          </details>
        )}
